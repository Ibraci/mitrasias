$( document ).ready(function() {
    ...
    setTimeout(function(){ Materialize.toast('Welcome to Mitras IAS', 4000) }, 4000);

    $('#pref').change(function(){
        $('#op').html("Information about price and payment is available <a href='http://mitrasias.com/program/' >here</a>");
    });

    $('#where').change(function(){
        var value = $('#where :selected').val();
        if(value == "internet"){

                console.log("int");
            $('#wh_op').html(
                '<div class="input-field col s12">'+
                    '<input id="icon_telephone" type="text" class="validate" name="whe_in">'+
                    '<label for="icon_telephone">Please specify the name of website or article</label>'+
                '</div>');

                // '<div class="form-group">'+
                //     '<label for="whe_in">Please specify the name of website or article</label>'+
                //     '<input type="text" name="whe_in" class="form-control" id="whe_in">'+
                // '</div>');
            ...
            $('#wh_op').html(
                ...
                '<div class="input-field col s12">'+
                    '<input id="icon_telephone" type="text" class="validate" name="whe_in">'+
                    '<label for="icon_telephone">Tell us something more about that advertisement</label>'+
                '</div>');



                // '<div class="form-group">'+
                //     '<label for="whe_in">Tell us something more about that advertisement</label>'+
                //     '<input type="text" class="form-control" name="whe_in" id="whe_in">'+
                // '</div>'
                // );
        } else {
            console.log("value");
